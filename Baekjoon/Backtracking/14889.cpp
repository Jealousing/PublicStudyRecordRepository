...
이제 N/2명으로 이루어진 스타트 팀과 링크 팀으로 사람들을 나눠야 한다. BOJ를 운영하는 회사 답게 사람에게 번호를 1부터 N까지로 배정했고, 아래와 같은 능력치를 조사했다. 
...
void Backtracking(int num, int cnt)
{
    ...
    if (cnt == n/2)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            {
                // 팀시너지 점수 
                if (check[i] && check[j]) teamStart += arr[i][j];
                if (!check[i] && !check[j] ) teamLink += arr[i][j];
            }
        }

        teamStart = abs(teamStart - teamLink);
        if (teamStart < minValue) minValue = teamStart;
        return;
    }
    for (int i = num; i < n; i++)
    {
