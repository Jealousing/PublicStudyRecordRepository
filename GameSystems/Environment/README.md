...
<!-- TODO: GIF 넣기 (mesh ↔ RT 비교, occlusion on/off 등) -->
<!-- TODO: 전체 영상 → [▶ YouTube](영상_링크) -->
...
시스템의 핵심은 **적설(snow)** 이며, 이를 구현하던 중 **mesh → RT(RenderTexture)** 로 전환한 것이 이 프로젝트의 중심 이야기입니다. 단순히 기능을 만드는 데 그치지 않고, **성능 한계에 부딪혀 접근 방식 자체를 바꾼 경험**이 이 시스템을 관통합니다.
...
가장 큰 문제는 **CPU 비용**이었습니다. 메시 세분화와 정점 처리가 CPU에 집중되었고, 작업을 **멀티코어로 분산해도 프레임 저하가 해소되지 않았습니다.** 적설 범위가 넓어질수록 CPU가 병목이 되어, 이 방향으로는 한계가 분명했습니다.

(청크 경계의 이음새, 세분화 과정의 T-junction 균열 같은 문제도 있었지만 부차적이었고, 핵심은 성능이었습니다.)

### 해결: 작업을 GPU로 옮기다

그래서 적설을 "메시"가 아니라 **"텍스처(RT) + 셰이더 변위"** 로 재설계했습니다.
...
> 결과: <!-- TODO: 프레임 개선 수치 (예: 평균 30 → 60fps, 프레임타임 N→M ms) -->
> mesh · RT 두 방식 모두 코드에 보존되어 있어, 시연 영상에서 직접 비교합니다.
> 상세 구현은 **[Weather/Snow/README.md](./Weather/Snow/README.md)** 참고.
...
이 비유 덕분에 "언제 무엇을 초기화하고, 무엇을 매 프레임 돌릴지"를 익숙한 감각으로 설계할 수 있었습니다.

---

## 시스템 아키텍처

`EnvironmentManager`가 컴포지션 루트로서 하위 시스템을 조립 · 구동합니다. 시간 / 날씨 / 눈 / 비는 서로 독립적인 모듈이며, 폴더 경계로 분리되어 있습니다.
...
### mesh → RT 전환
적설을 RenderTexture + 셰이더 변위로 재설계하여 CPU 부하를 GPU로 이전했습니다. 이 시스템의 중심 의사결정입니다. → [Snow](./Weather/Snow/README.md)

### Sky Occlusion
천장 아래나 실내에는 눈 · 비가 닿지 않아야 자연스럽습니다. 이를 위해 **BFS 거리장**을 계산해 "하늘이 얼마나 열려 있는가"를 구하고, **5×5 타일 아틀라스 텍스처**에 구워 셰이더에서 소비합니다. 표면 셰이더는 이 값으로 천장 밑 적설을 잘라냅니다. → [Snow](./Weather/Snow/README.md)
