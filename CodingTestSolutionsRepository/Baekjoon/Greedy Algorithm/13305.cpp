...
long long roadDistance[MAXSIZE], price[MAXSIZE], n;
long long temp, minSum = 0;
...
int main()
{
    ...
    // 처음 출발할 때 자동차에는 기름이 없어서 주유소에서 기름을 넣고 출발하여야 한다.  -> 주유가격 저장
    temp = price[0];
    // 두번째 도시까지의 거리만큼 비용 지불 (최소)
    minSum = price[0] * roadDistance[0];
    ...
    for (int i = 1; i < n; i++)
    {
        ...
        // 지정된 가격으로 다음 도시까지의 거리만큼 비용 지불
        minSum += temp * roadDistance[i];
    }
