...
결국 담당판사를 설득하는 데에 성공하였다. 담당판사는 다음과 같은 판결을 내렸다. “ 6.25 이전의 개인소유 대지들은 99%가 남북, 동서 방향으로 평행한 직사각형 모양이었으므로, 
...
int main()
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		if (minX > checkX) minX = checkX;
		if (maxX < checkX)maxX = checkX;

		if (minY > checkY)minY = checkY;
		if (maxY < checkY)maxY = checkY;
	}
