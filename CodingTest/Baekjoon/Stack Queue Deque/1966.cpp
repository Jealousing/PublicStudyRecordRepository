...
int main()
{
	...
	for (int i = 0; i < t; i++)
	{
		...
		{
			cin >> temp;
			Queue.push(make_pair(j, temp));
		} 

		while (true)
		{
			int maxValue = 0, maxsize = Queue.size();
			queue<pair<int, int>> temp = Queue;

			for (int k = 0;k < maxsize; k++)
			{
				if (temp.front().second > maxValue)
				{
					maxValue = temp.front().second;
				}
				temp.pop();
			}
			...
			for (int k = 0; k < maxsize; k++)
			{
				if (Queue.front().second != maxValue)
				{
